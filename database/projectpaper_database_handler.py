...
def assign_paper_to_project(
    ...
    cursor.execute(
        """INSERT INTO paperprojects_table (project_id, paper_hash, summary, newsletter, seen, is_replacement) VALUES (%s, %s, %s, %s, %s, %s)
...
def set_newsletter_tags_for_project(
    project_id: str, paper_hashes: list, summaries: list
):
    """
    ...
    values = [
        (project_id, paper_hash, summary, True, False)
        for (paper_hash, summary) in zip(paper_hashes, summaries)
    ]
