...
def test_user_profile_embeddings():
    ...
    response = requests.post(f"{BASE_URL}/api/createProject", json=project_data, timeout=10)
    ...
    response = requests.post(f"{BASE_URL}/api/user-profile/create-embedding", json=user_profile_data, timeout=10)
    if response.status_code == 200:
    ...
    response = requests.post(f"{BASE_URL}/api/createProject", json=project_data2, timeout=10)
    if response.status_code == 200:
    ...
    response = requests.post(f"{BASE_URL}/api/user-profile/create-embedding", json=user_profile_data2, timeout=10)
    if response.status_code == 200:
        result = response.json()
    ...
    response = requests.post(f"{BASE_URL}/api/user-profile/find-similar", json=similar_data, timeout=10)
    if response.status_code == 200:
        result = response.json()
    ...
    response = requests.get(f"{BASE_URL}/api/user-profile/similarity-matrix", timeout=10)
    if response.status_code == 200:
        result = response.json()
