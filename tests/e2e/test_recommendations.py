...
class TestRecommendationsFlow:
    ...
    def test_create_project_and_get_recommendations(self, page, test_project_data):
        ...
        try:
            page.wait_for_selector(".recommendation-card", timeout=90000)
            ...
            pytest.fail(f"Papers failed to load within timeout period. Page text: {error_text[:500]}")
    ...
    def test_paper_metadata_display(self, page, test_project_data):
        ...
        # Verify authors (p element containing 👥 emoji)
        authors = first_paper.locator("p:has-text('👥')")
        assert authors.count() > 0, "Authors not found"
    ...
    def test_pdf_and_open_access_buttons(self, page, test_project_data):
        ...
        if pdf_button.count() > 0:
            ...
            assert "007bff" in button_html.lower(), "PDF button should have blue styling"
    ...
    def test_read_paper_link(self, page, test_project_data):
        ...
        assert "window.open" in read_link_html, "Read Paper link doesn't open new window"
    ...
    def test_search_papers(self, page, test_project_data):
        ...
        if search_input.count() > 0:
            ...
            assert filtered_papers.count() <= initial_count, "Search returned more papers than initial"
    ...
    def test_paper_count_matches_request(self, page, test_project_data):
        ...
        assert actual_count >= requested_count - 2, f"Expected ~{requested_count} papers, got {actual_count}"
        assert actual_count <= requested_count + 2, f"Expected ~{requested_count} papers, got {actual_count}"
